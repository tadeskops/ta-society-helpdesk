...
| Mobile wheel-picker | `FEATURE_TSH_VEHICLES_WHEEL_PICKER` (`false`) | — (client-only UI mode) | n/a | **Client-only, no server impact.** When the flag is ON *and* the viewport is `≤ 640 px` *and* the caller is a non-editor (RESIDENT / CONTRIBUTOR / UNKNOWN), `vehicles.html` renders three iOS-style scroll wheels (tower / floor / unit) instead of the keyboard search field. When OFF (the default) the mobile experience stays keyboard-first search + result card (Design 5), which is the least-overwhelming baseline. Editors always see the seat-map schematic regardless of the flag — the wheel picker is a resident-facing entry aid only. Mock-up: `temp/vehicles-design-10-wheel-picker.html`. |
