...
| UI      | `docs/vehicles.html`: single **Google-style pill search box** (centered, `max-width: 400px`, `border-radius: 999px`, gold magnifying-glass icon on the left, native `type="search"` × for clear) with placeholder "Search by Flat or Vehicle number". Autofocus, live 100 ms debounced filter, ≥ 2 chars. **Search is format-tolerant** — case-insensitive, and any non-alphanumeric character (spaces, hyphens, dots, slashes) is stripped from both the query and the stored keys before matching. Flat numbers canonicalise via `parseFlat` + `flatFromParts` so `A304`, `A0304`, `a-304`, `A 0304`, and `a/304` all resolve to the same stored flat. Vehicle numbers are compared as substrings on the server-normalised `regNo` (already uppercase-alphanumeric), so `MH-11-JJ-0234`, `mh 11 jj 0234`, and `MH11JJ0234` all match. When a flat-shaped query has zero registered vehicles, the client also tries a vehicle substring pass on the same normalised input and surfaces those hits instead of dead-ending. Result card shows the matched flat + type + optional sticker + comments. **Manage pane (editors only)** is a pictorial seat-map view: one collapsible `<details>` per tower, rows = floors (top-down), columns = units. Tower cards flow in a **flex-wrap shelf** — each card takes its natural content width, so a 4-unit tower (A/C) is narrower than a 6-unit tower (B) while **every seat cell is exactly `--veh-cell-w` wide (default `3.1rem`, `2.7rem` under 480 px)** — the boxes stay the same size across towers. Cards wrap onto the next row when the row runs out of horizontal space. Row headings show the floor number as a small gold-tinted chip (no `F` suffix — `G` for ground, plain number otherwise); column headings show the unit number `01`, `02`, … in the same chip style so the row/column rails read as a light coloured index rather than heavy text. Above the schematic, a shared `.veh-manage-controls` flex row places the **Tower filter chip row** ("All" + one chip per tower letter from `cfg.lists.towers`) and the **cell-colour legend** side-by-side on desktop (stacks on mobile below 640 px); filter selection persists per-browser via `localStorage['tsh_vehicles_tower_filter']`. Narrowing to a single tower sets `#vehTowers[data-single-tower="1"]` so the remaining tower spans the full row. **Picking a flat opens the reusable `UI.SidePanel` drawer** (see §14.6a) titled `Flat <id>` — right-docked on desktop, bottom sheet on phones — containing the read-only vehicle roster (one row per vehicle: reg + type + sticker + comments + emails) plus an inline Add form. The `#vehFlatDetail` DOM node is moved into the panel body on open and restored to `#vehManage` on close, so its delegated click/input/submit listeners survive intact. **Editing an existing vehicle** also opens `UI.SidePanel` (single-vehicle edit form, unchanged). All writes go through `PUT /vehicles` (whole-list) except one-shot deletes which use `DELETE /vehicles/:id`. |
...
| Future v2 | See §14.10a below. Six capabilities are pre-designed: per-caller email filter, sticker-only PATCH for a future SECURITY_GUARD role, bulk email upload/parser, resident self-registration (post id-validation), curated member allowlist (Gmail-only for now), and printable-report affordance. All six have reserved feature flags (default `false`) and reserved typed config under `system.vehicles` so activation is a Settings toggle, not a code change. |
...
| Curated member allowlist | `FEATURE_TSH_VEHICLES_MEMBER_ALLOWLIST` (`false`) | `GET`/`PUT`/`DELETE /vehicles` (transparent) | `system.vehicles.memberAllowlist` (list of e-mail strings, case-insensitive) + `system.vehicles.memberAllowlistEditorRoles` (who may curate the list; default `ADMIN`/`CHAIRMAN`/`SECRETARY`) | **Wired.** When on, non-editor callers whose signed-in e-mail is not in `memberAllowlist` receive `403`. Editors always bypass. Empty list + flag on = editors only. Managed from Settings by roles in `memberAllowlistEditorRoles` (list-management UI is future work). |
| Report Print | `FEATURE_TSH_VEHICLES_REPORT_PRINT` (`false`) | client-only affordance in `docs/vehicles.html` | `system.vehicles.editorRoles` (only editors see the button) | **Wired.** `GET /vehicles` payload advertises the flag as `reportPrintEnabled: boolean`. When on and the caller is an editor, a "Print report" button appears above the tower grid; clicking it renders a print-friendly summary table (uses the same `typeLabel()` helper as the UI so EV variants read as "4-wheeler EV", "2-wheeler EV"). Off by default — the print layout is client-only. |
