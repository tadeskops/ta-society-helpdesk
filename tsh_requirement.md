...
| UI      | `docs/vehicles.html`: single search box (autofocus, uppercase, live 100 ms debounced filter, ≥ 3 chars) — this is the primary resident/PUBLIC affordance (Design 5, search-first). Result card renders the matched flat + type + optional sticker + comments. **Manage pane (editors only)** is a pictorial seat-map view (Design 1 refinement): one collapsible `<details>` per tower, rows = floors (top-down), columns = units. **Each occupied cell is a rectangular tile with a mini stacked bar-chart footer** — blue segment = 4-wheeler count, green segment = 2-wheeler count, split proportionally when both types are present (Design 1: "two colours mean both 2W and 4W"). Above the schematic, a **Tower filter chip row** ("All" + one chip per tower letter from `cfg.lists.towers`) narrows the view to a single tower; selection persists per-browser via `localStorage['tsh_vehicles_tower_filter']`. Clicking a cell opens the flat-detail panel, which now contains **only the read-only information grid** (one row per vehicle: reg + type + sticker + comments + emails) plus an inline Add form. **Editing an existing vehicle opens the reusable `UI.SidePanel` drawer** (see §14.6a) — right-docked on desktop (≥ 641 px), bottom sheet on phones. On mobile (≤ 640 px) the search-first surface remains the default entry point for residents — an iOS-style wheel-picker alternative is reserved as a v2 hook (`FEATURE_TSH_VEHICLES_WHEEL_PICKER`, default OFF, keyboard-search stays the default). All writes go through `PUT /vehicles` (whole-list) except one-shot deletes which use `DELETE /vehicles/:id`. |
...
| Future v2 | See §14.10a below. Five capabilities are pre-designed: per-caller email filter, sticker-only PATCH for a future SECURITY_GUARD role, bulk email upload/parser, resident self-registration (post id-validation), and an iOS-style wheel picker for mobile residents. All five have reserved feature flags (default `false`) and reserved typed config under `system.vehicles` so activation is a Settings toggle, not a code change. |
...
| Mobile wheel-picker | `FEATURE_TSH_VEHICLES_WHEEL_PICKER` (`false`) | — (client-only UI mode) | n/a | **Wired.** Client-only, no server impact. When the flag is ON *and* the viewport is `≤ 640 px` *and* the caller is a non-editor (RESIDENT / CONTRIBUTOR / UNKNOWN), `vehicles.html` renders a tab-strip toggle above the search hero — "Keyboard search" (default) and "Wheel picker". Selecting the second option reveals three iOS-style scroll wheels (tower / floor / unit) with a gold selection band, populated from `Flags.raw.lists.towers` + the first tower's `towerLayouts` dims. The selected flat's roster renders in a compact result card beneath the wheels. When OFF (the default) the toggle is hidden entirely and the mobile experience stays keyboard-first (Design 5). Editors always see the seat-map schematic regardless of the flag — the wheel picker is a resident-facing entry aid only. Mock-up: `temp/vehicles-design-10-wheel-picker.html`. |
